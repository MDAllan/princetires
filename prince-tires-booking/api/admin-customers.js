...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Fetch customers with wholesale-pending OR wholesale tags
    const [pendingData, approvedData] = await Promise.all([
      shopifyGet(token, 'customers/search.json?query=tag:wholesale-pending&limit=250&fields=id,first_name,last_name,email,phone,tags,note,created_at,orders_count,state,metafields'),
    ...
    // For approved customers, fetch their metafields
