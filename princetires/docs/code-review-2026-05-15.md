...
- [~] **C3 `[theme]`** — Partial 2026-05-18: dead `schema-review.liquid` (rendered nowhere) deleted. Still open: `geo-meta` vs the `product-reviews` section both emitting a `Product` node. — Product pages emit up to 3 conflicting `Product` JSON-LD blocks with different `aggregateRating` values. `geo-meta.liquid`, `product-reviews.liquid`, `schema-review.liquid` — keep `geo-meta.liquid` as the single Product node, fold the review array into it, delete the others' output.
