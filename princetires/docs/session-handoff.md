...
- **2 wheel stragglers (2026-05-20)** — `Armed Offroad Havoc Wheels` (no specs in title at all) and `Envy 546GM EV-5 16" Gun Metal Alloy Wheel` (diameter only) couldn't be parsed by `db/fill-wheel-fitment.mjs`. Look up supplier spec sheets and manually populate the 4 fitment metafields in the admin product page, or de-list. Other 16 steel/trailer wheels are missing `rim_offset` only — that's accurate (steel wheels are typically universal-fit), no action needed unless owner says otherwise.
...
- **Header nav add (2026-05-20 — USER TASK, ~1 min)** — under the **Tires** menu in Shopify Admin → Online Store → Navigation → Main menu, add "All-Weather Tires" → `/pages/all-weather-tires-calgary` and "All-Season Tires" → `/pages/all-season-tires-calgary`. Highest-authority remaining inbound link for the new landing pages. I can't do this — Shopify Admin UI only.
- **Footer "Popular pages" visual QA (2026-05-20)** — new column added between dynamic blocks and newsletter in `sections/pt-footer.liquid:108-117`. Once CDN cache flushes, confirm the grid doesn't overflow at 900–1200px breakpoints. The grid is `repeat(N, 1fr)` at desktop — was 3–4 cols, now 4–5. If it looks tight, narrow the breakpoint thresholds in `.pt-footer__columns` media queries.
...
- **Push commit `edff591` to origin (2026-05-20)** — today's SEO landing-page network commit on `phase-4-wholesale-portal`. NOT pushed; user didn't ask. 13 files / 1069 insertions / 93 deletions. Will need a push (or merge to `master`) when ready.
