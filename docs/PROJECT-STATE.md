...
| **3.5 — Service catalog** | "to build" | 🟡 **Stage 1 built 2026-05-15** | DB `services` table + `/admin/services` price/duration editor + `GET /api/services`. ⏳ Needs migration 015 applied + app deploy. Stage 2 (theme rewire so edits reach the storefront) pending. See `docs/phase-3.5-service-catalog.md`. |
